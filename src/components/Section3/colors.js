...
export const getNodeColorByState = (visState, graphData) => {
  ...
  switch (visState) {
      ...
      let linkedNodes = [
        ...new Set(
          links
            .filter((d) => {
              return (
                [spikePMCID, modernaPMCID, pfizerPMCID].includes(d.source.id) ||
                [spikePMCID, modernaPMCID, pfizerPMCID].includes(d.target.id)
              );
            })
    ...
    case 'state6':
      fn = (node) =>
        node.id === spikePMCID ? 'tomato' : 'rgba(255, 255, 255, 1)';
      break;
